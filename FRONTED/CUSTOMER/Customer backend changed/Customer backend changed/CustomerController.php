...
class CustomerController
{
    ...
    */
    public function updateMe()
    {
        ...
        $allowedFields = [
            ...
            'date_of_birth', 'anniversary_date','preferences'
        ];
    ...
    */
    public function getMyAppointments()
    {
        ...
        // Get total count
        $countSql = "SELECT COUNT(*) FROM appointments WHERE customer_id = ? AND salon_id = ?";
        $countStmt = $this->db->prepare($countSql);
        $countStmt->execute([$customerId, $salonId]);
        $total = $countStmt->fetchColumn();

        // Pagination
        $offset = ($page - 1) * $limit;
        $sql .= " ORDER BY a.appointment_date DESC, a.start_time DESC LIMIT $limit OFFSET $offset";
        ...
        foreach ($appointments as &$appointment) {
            ...
            $stmt = $this->db->prepare("
                ...
                INNER JOIN staff_info st ON asvc.staff_id = st.staff_id -- staff table join (updated line)
            ...
            // Get packages
            $stmt = $this->db->prepare("
                SELECT ap.package_id, p.package_name, ap.staff_id,st.name AS staff_name
                FROM appointment_packages ap
                INNER JOIN packages p ON ap.package_id = p.package_id
                INNER JOIN staff_info st ON ap.staff_id = st.staff_id -- staff table join (updated line)
    ...
    */
    public function getAppointments($customerId)
    {
        ...
        // Pagination
        $offset = ($page - 1) * $limit;
        $sql .= " ORDER BY appointment_date DESC, start_time DESC LIMIT $limit OFFSET $offset";
    ...
    */
    public function getMyFeedback()
    {
        ...
        // Get total count
        $countSql = str_replace("af.*, a.appointment_date", "COUNT(*)", $sql);
        $countStmt = $this->db->prepare($countSql);
        $countStmt->execute($params);
        $total = $countStmt->fetchColumn();
        ...
        // Pagination
        $offset = ($page - 1) * $limit;
        $sql .= " ORDER BY af.created_at DESC LIMIT $limit OFFSET $offset";
